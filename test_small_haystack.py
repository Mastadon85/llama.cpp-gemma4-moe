...
def generate_haystack(num_tokens, needle, needle_pos, question):
...
def main():
    ...
    needle_pos = 150000
    question = "Question: What is the secret ingredient for the perfect Attention Matching? Answer:"
    
    filename = "small_haystack.txt"
    print(f"Generating {num_tokens} token haystack in {filename}...")
    
    content = generate_haystack(num_tokens, needle, needle_pos, question)
    ...
    test_cmd = f".\\llama.cpp\\build\\bin\\Release\\llama-cli.exe -m {model_path} -f {filename} -n 32 --compact --compact-ratio 10 --am-trigger 100000 --am-gpu -ngl 99"
